...
export const POSIX_OPENCLAW_TMP_DIR = "/tmp/rcclaw";
...
export function resolvePreferredOpenClawTmpDir(
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "rcclaw" : `rcclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[rcclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback RCclaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback RCclaw temp dir: ${fallbackPath}`);
    }
