...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "rcclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "rcclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      ...
      .catch((error) => {
        console.error(
          "[rcclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[rcclaw] ${parsed.error}`);
      process.exit(2);
    }
