            ...
            if not selected:
                ...
                best = max(
                    ...
                    key=lambda d: (
                        ...
                        - (1 - lambda_param)
                        * max(cos_sim(d.document.embedding, s.document.embedding) for s in selected)
                    ),
                )
...
class EnhancedRAGEngine:
    def __init__(
        self, embed_fn: Callable[[str], List[float]], llm_fn: Optional[Callable[[str], str]] = None
    ):
