...
class VisionEngine:
    ...
    def _load_ocr(self):
        if self._ocr is None:
            try:
                ...
                self._ocr = PaddleOCR(
                    use_angle_cls=True, lang="ch", use_gpu=self._use_gpu, show_log=False
                )
    ...
    def recognize_text(self, image_bytes: bytes) -> OCRResult:
        ...
        if self._ocr == "fallback" or self._ocr is None:
            return OCRResult(
                blocks=[], full_text="", image_hash=img_hash, error="OCR not available"
            )
        ...
        try:
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "请对比这两张界面截图，详细说明发生了哪些功能性或语义上的变化（例如：新增了弹窗、按钮位置改变、进入了新页面等）。",
                            },
                            {
                                "type": "image_url",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self._vlm_api_url, json=payload, headers=headers, timeout=15
                ) as resp:
                    if resp.status == 200:
