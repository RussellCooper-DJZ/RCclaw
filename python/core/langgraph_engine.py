...
class LangGraphAgent:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges(
            ...
            {"direct": "synthesizer", "research": "researcher", "error": END},
        )
        ...
        workflow.add_conditional_edges(
            ...
            {"human": "human_review", "final": "finalizer", "error": END},
        )

        workflow.add_conditional_edges(
            "human_review",
            self.route_after_hitl,
            {"final": "finalizer", "replan": "planner", "error": END},
        )
    ...
    async def node_planner(self, state: AgentState) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    '你是一个任务规划专家。请将用户的请求拆分为具体的执行步骤。如果问题很简单，只需一步。返回 JSON 格式：{{"plan": ["step1", "step2"]}}',
                ),
                ("user", "{query}"),
            ]
        )
    ...
    async def node_synthesizer(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"draft_answer": draft, "confidence": confidence, "hitl_required": hitl_required}
        except Exception as e:
            return {"error": str(e)}
    ...
    async def run(self, query: str, session_id: str) -> Dict[str, Any]:
        ...
        if state_snapshot.next and "human_review" in state_snapshot.next:
            logger.info(
                f"Session {session_id} requires human review. Draft: {state_snapshot.values.get('draft_answer')}"
            )
    ...
    async def submit_human_decision(self, session_id: str, decision: str, feedback: str = ""):
        ...
        self.graph.update_state(
            config, {"hitl_decision": decision, "hitl_feedback": feedback}, as_node="human_review"
        )
        ...
        state_snapshot = self.graph.get_state(config)
        return {"status": "completed", "answer": state_snapshot.values.get("final_answer")}
