...
```bash
export RCCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.rcclaw/rcclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env RCCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `rcclaw` wrapper for guest `message send/read`; `node rcclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/rcclaw-parallels-smoke.*`
...
- host inbound nonce appears in `rcclaw message read` output
